...
export const setPriorityColor = (taskElement) => {
    ...
    const priorityElement = taskElement.querySelector(".task-item__priority");
...
export const arrangeAccordingToOrder = () => {
    ...
    taskLi.forEach(task => {
        ...
        Task.innerHTML = `
        <div class="task-item">
            <div class="task-item__information">
                <div class="task-item__priority task-item__priority--${task.priority.toLowerCase()}">${task.priority}</div>
                <div class="task-item__date">${task.date}</div>
                <ul class="task-item__responsibles">${task.responsabilities.map((responsible) =>`<li class="task-item__responsible-name">${responsible}</li>`).join("")}</ul>
            </div>
            <div class="task-item__description">
                <h2 class="task-item__title">${task.title}</h2>
                <p class="task-item__text-description">${task.description}</p>
            </div>
        </div>

        <div class="task-item__actions">
            <svg class="task-item__delete-icon bi bi-trash3" data-id="${task.id}" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16">
                <path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5M11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47M8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5"/>
            </svg>

            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="task-item__check-icon bi bi-check-circle-fill" viewBox="0 0 16 16">
    ...
    document.querySelectorAll(".task-item__delete-icon").forEach(trash => {
    ...
    document.querySelectorAll(".task").forEach(setPriorityColor)
}

//Função de criar fomulário
export const createFormAdd = () => {
    const title_form = document.getElementById("task-title").value;
    const priority = document.getElementById("priority").value;
    const date = document.getElementById("task-date").value;
    const description = document.getElementById("task-description").value;

    const responsabilitys = [];
    const checkboxes = document.querySelectorAll("input[name='responsibility']:checked");
